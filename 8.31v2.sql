...
select id from emp where namea='canlu';
select name from emp where id=4;
select name from emp where id=(select id dfrom emp where name="cnalu");
select enterydate from emp where name='canlu';
select name from emp where enterydate>any(select enterydate from emp where name='canlu');
USE CANLU;
...
INSERT ACCOUNT VALUES (1,'zs',1808000),(2,'ls',8000500),(3,'zw',5155550);
select @@autocommit;
select @@autocommit=0;
select * from account;
select money from account where name='zs';
update account set money=money-1000 where name='zs';
update account set  money=money+1000 where name='ls';
select money from account where name='ls';
commit;
rollback ;
start transaction ;
commit;
...
select @@transaction_isolation;
/*概述-》sql——》函数-》约束->多表查询-》事务{主要都是记忆代码和概念，要定期看笔记复习}